...
// FETCHED DATA (SSR OR LOCAL JSON)
import blogData from "@/public/data/blog.json"; // MOVE DATA INTO JSON

export default function blogPage() {
  // EXTRACT DATA
  const { posts, categories, trending } = blogData;

  // NEW FEATURED → MOST RECENT
                  ...
                  {visiblePosts.map((post) => (
                    <Link href={`/blog/${post.id}`} key={post.id}>
                      <motion.div
                        ...
                        className="bg-white rounded-3xl shadow-sm overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col"
                      >
                        {/* IMAGE */}
